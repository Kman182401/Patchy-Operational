...
This is a **routing layer** between Patchy Bot's runtime code and the 9 installed Python skills (`python-error-handling-skill`, `python-resilience-skill`, `python-resource-management-skill`, `python-observability-skill`, `python-type-safety-skill`, `python-anti-patterns-skill`, `python-design-patterns-skill`, `python-code-style-skill`, `writing-python-skill`).
...
| `telegram-qbt/patchy_bot/runners/*` (completion poller, schedule runner, movie-track runner, remove-runner, pending-monitor) | `python-resilience` | `python-observability`, `python-error-handling` | Long-running async loops, idempotency, retry/backoff, structured logging for runner state |
...
| `telegram-qbt/patchy_bot/config.py` and env-var loading | `python-type-safety` | `python-anti-patterns`, `writing-python` | Plain `os.getenv` + typed conversion, validate at startup |
...
| Adding a new background loop, scheduler, queue, or worker | `python-resilience` | `python-observability`, `python-error-handling` | Idempotency rules, retry/backoff, dead-letter handling — extend the existing `runners/` system |
...
2. **Don't introduce `pydantic-settings`** without explicit user approval. Patchy Bot uses plain `os.getenv` + `config.py`; if you want a typed-settings library, ask first.
3. **Don't introduce `Celery`/`RQ`/`Dramatiq`.** Patchy Bot has its own runner system (`runners/`) — apply queue patterns (idempotency, dead-letter, retry/backoff) to the existing runner code, not by adding a new dependency.
...
- "I'm adding a runner / scheduler / poller" → `python-resilience` + `python-observability` always
- "I'm adding new env config" → `python-type-safety` always
