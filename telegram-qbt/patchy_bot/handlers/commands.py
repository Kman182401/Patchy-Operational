...
async def cmd_profile(bot: Any, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [
        ...
        "• quality: <code>1080p preferred</code> (auto-fallback)",
...
async def cmd_unlock(bot: Any, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        pass
    await bot._navigate_to_command_center(msg, uid)
...
async def on_cb_menu(bot_app: Any, *, data: str, q: Any, user_id: int) -> None:
    ...
    if data == "menu:schedule":
        ...
        if tracks:
            ...
            total_tracking = len(tracks) + len(movie_tracks_cb)
            rows: list[list[InlineKeyboardButton]] = [
                [
                    InlineKeyboardButton("\U0001f4fa Add New Show", callback_data="sch:addnew"),
                    InlineKeyboardButton("\U0001f3ac Add New Movie", callback_data="msch:add"),
                ],
                [InlineKeyboardButton(f"\U0001f4cb Active Tracking ({total_tracking})", callback_data="sch:myshows")],
            ]
    ...
    if data == "menu:profile":
        ...
        lines = [
            ...
            "\u2022 quality: 1080p preferred (auto-fallback)",
