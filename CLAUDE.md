...
- Use the `context7` MCP plugin (the user holds an active Context7 Pro subscription, so call it freely) for library API documentation before writing code that uses unfamiliar or recently-updated library functions. Context7 also indexes Claude Code skill libraries — query it when looking for new skills to install.
